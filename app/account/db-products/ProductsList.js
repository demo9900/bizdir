...
const ProductsList = () => {
  ...
  const getProductsData = async () => {
    try {
      ...
      const res = await fetch(
        process.env.BACKEND_URL + "/api/product/user/" + session?.user?.id,
        {
          headers: {
  ...
  const deleteProduct = async (id) => {
    try {
      const res = await fetch(process.env.BACKEND_URL + "/api/event/" + id, {
        headers: {
          authorization: "Bearer " + session.jwt,
        },
        method: "DELETE",
        ...
        {productData.map((product, idx) => {
          return (
            <tr key={product._id}>
              <td>{idx + 1}</td>
              <td>
                <img alt={product.product_name} src={product.images[0]} />
              ...
              <td>
                <span className="db-list-rat">
                  {!!product.product_views ? product.product_views : 0}
                </span>
              </td>
